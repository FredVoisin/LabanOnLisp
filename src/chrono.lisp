...
(defmethod find-situation-danser ((self chronometre))
 (let ((situation-windows (windows :class 'situation-window))
       w)
...
(defmethod durée ((self situation))
  (datum (find-if #'(lambda (x) (eq (class-of x) (find-class 'durée))) 
                  (data self))))


(defmethod set-duration ((self situation-window) (duration number))
  (setf (datum (find-if #'(lambda (x) (eq (class-of x) (find-class 'durée))) 
                        (data (situation self)))) duration)
  (set-dialog-item-text 
   (view-named 'data
               (find-if #'(lambda (x) (eq (view-nick-name x) 'durée))
