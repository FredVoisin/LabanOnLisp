...
;;; SAVE-OBJECT, Version 10A (EXPERIMENTAL, patched),
;;; Effective Date: April 1995.
;;; Copyright (C) Kerry V. Koitzsch, 1992,1993,1994,1995.
...
--- ccl (PCL) instances, and ccl(PCL) classes.
--- hash tables.
...
--- CLIM objects (saved as ordinary ccl instances)
...
Versions of PCL/ccl: AAAI PCL, Victoria Day PCL, REV 4b PCL, Lucid 4.0
ccl, Lucid 4.1 ccl.
...
(1) Redefine the IN-PACKAGEs below to suit: they should USE ccl or PCL,
though.
...
(with-saved-objects (x "my-multiple-save-file.lisp")
    ...
    (make-instance 'my-class) ;; a ccl/pcl instance...
...
--- the ability to save ccl instances with unbound slots: fixed the
...
#+lucid
(in-package 'DATABASE :nicknames '(DB) :use '(ccl lisp))
...
(ccl::old-in-package :COMMON-lisp-USER :nicknames '(USER)) ;;; added this for version X.
  ...
  (pushnew :ccl *features*) ;; MCL has ccl, but it isnt in the features list.

  (unless (find-package 'ccl)
    (make-package 'ccl :use '(ccl common-lisp))) ;;hence no cl package, either
...
(eval-when (load eval compile)
  ...
  (when (find-package 'ccl)
    (pushnew :ccl *features*))

  (unless (find-package 'database)
    (make-package 'database :nicknames '(db) :use '(ccl)))

  (shadowing-import '(setf documentation) 'database)

  (unless (find-package 'ccl)
    (format t "Couldnt find the ccl package, trying to continue.~%"))
...
;;; make sure you USE-PACKAGE your favorite brand of ccl or PCL, though.
...
(defvar *debug-instance-storage* nil
  "when this one is T, status messages are printed by the ccl instance saver 
...
(defvar *save-object-system-date* 
    "July 1994 Save Object 9 X.3 Experimental (PATCHED).")
...
(defvar *seen* nil "when using linear search, where the ccl instances go.")
(defvar *vars* nil)
...
(defvar *vendor-defstruct-slot-desc-index-table* nil
 ...
 on desc (which is presumed to be a ccl instance) and the slot name.
        ...
        (if default-slot NIL ;; do nothing.
          (if (not unbound-slot)
              (cond ((instance-p i)
                    ...
                    ((structurep i)(set-defstruct-slot-value i name thang)))
  ...
  `(let ((.stream-abort-flag. :ABORT))
     (unwind-protect
         (multiple-value-prog1
           ...
           (when ,so-var (ccle ,so-var :abort .stream-abort-flag.))))))
...
(defmacro WITH-SAVED-OBJECTS ((svar sspec . options) &body body)
 ...
 NOTE: This macro now returns the filename where the data is stored."
  ...
  (setf (lisp:get 'already-exists 'file)(probe-file sspec))
...
(defun cclUREP (x)
  "This machine dependent predicate returns t if the object <x> is
   a lexical cclure, ie. either a (function (lambda .... thing, or
   a hash-mark quote thing,"
  #+lucid (and (not (typep x 'compiled-function))
               (typep x 'system:procedure))
  #+allegro-v4 (typep x 'excl::cclure)
  #-(or lucid allegro-v4)(progn x nil)
  )

(defun cclURE-DUMP-FORM (cclure)
  #-:function-lambda-expression (declare (ignore cclure))
  #+:function-lambda-expression
  (multiple-value-bind (lambda-expression cclure-p name)
      (function-lambda-expression cclure)
...
;;; in Allegro-V4
#+:ignore 
(defun cclURE-DUMP-FORM-V70 (cclure)
  (let ((ans nil)
        (strname ""))
    (setq *readtable* (copy-readtable))
    (set-dispatch-macro-character #\# #\' (function pseudo-quote-reader))
    (set-dispatch-macro-character #\# #\< (function pseudo-quote-reader))
    (setf strname (format nil "~S" cclure))
...
(defun INSTANCE-DUMP-FORM (instance)
  "Basic dump form for ccl/pcl instances. checks if the instance has a custom
...
(defun COMPILED-FUNCTION-DUMP-FORM (X)
  ...
  #+lispm (if (si:lexical-cclure-p x) nil)
...
(eval-when (load eval compile)
  ...
  (defun CLASS-SLOTNAMES (class-object)
    "Calls the ccl internal function to compute class slot names."
...
#+ignore
  ...
  (defmethod ALL-SLOTNAMES ((instance T) &optional (all-allocations T))
    ...
    (lisp:REMOVE NIL (mapcar #'ccl::slot-definition-name 
                        (class-slots (ccl::class-of instance)))))
  ...
  (defun GET-SUPERCLASS-NAMES (class)
    ""
    (mapcar #'ccl::class-name (ccl::class-direct-superclasses class)))

  ) ;; *END OF MCL CCL ccl eval-when! ****


;;; A ccl eval-when for Allegro PC:
...
(eval-when (load eval compile)
  ...
  (defun %CLASS-NAME (x)
    ...
    (if (instance-p x)(ccl::class-name (ccl::class-of x))
      (ccl::class-name x)))
...
(eval-when (load eval compile)
  ...
  (defun CLASS-SLOTS (class-object)
    "Calls the ccl internal function to compute class slot objects:
     used in CMUCL, too!"
  ...
  (defun %ALLOCATE-INSTANCE (class-object &rest htab-plist)
    (cond
     ...
     ((structurep class-object)(allocate-struct class-object))
  ...
  (defun INSTANCE-NAME (instance)
    ...
    (cond ((hash-table-p instance) 'hash-table)
          ((structurep instance)(get-defstruct-name instance))
...
#+CIRCULAR-TESTS ((circular-dotted-list-p instance)(circular-dotted-list-dump-form instance))
        ...
        ;; application dumping cclures just caused me to have unreadable
        ;; forms, so I've basically stubbed out cclures (as nil) above.
        ;; I'm unconvinced these can be written portably, do with this what
        ;; you want.
        ((cclurep instance) (cclure-dump-form instance))
...
(if (circular-list-p l)(flatten (get-circular-list-elements l))
  (let ((answers nil))
    (dolist (cell l answers)
      (setf answers (NCONC answers cell))) ;;; was NCONC.
    answers)))
...
""
(if (circular-list-p cells)(flatten1 (get-circular-list-elements cells))
  (let ((answer nil))
    (dolist (cell cells answer)
      (setf answer (NCONC answer cell))) ;;; was NCONC.
    answer)))
...
(defun GET-ORDERED-SLOT-VALUES (i)
  ...
  (cond ((instance-p i)(alphabetize-by-keyword (get-slot-values i)))
        ((structurep i)(get-defstruct-values i))
...
(defun NEWSYM (symbol)
  ...
  (read-from-string (concatenate 'string (string symbol)
                                 (format nil "~S" 
                                         (incf (lisp:get symbol 'namecounter))))))
...
(defun GET-DEFSTRUCT-TYPE (structname)
  ""
  (if (structurep structname)(type-of structname)
...
#-aclpc
(eval-when (load eval compile)
  (setf (symbol-function 'structurep) #'structurep)
  ) ;; end of eval-when....
|#

(defun GET-DEFSTRUCT-DESCRIPTOR (structname)
  ""
  (when (structurep structname)
...
"NEW: instead of re-calculating the slotnames every time, and sorting them alfabetically,
...
(or (gethash (instance-name i) *ordered-slot-name-htab*)
    (let ((them (lisp:remove nil (lisp:sort (all-slotnames i) #'sym<))))
...
(defun GET-ORDERED-SLOT-NAMES (i)
  ...
  (cond ((instance-p i)(lookup-ordered-slot-names i))
        ((structurep i)#-excl (get-defstruct-slot-names i)
                        #+excl (%get-defstruct-slot-names i)
                        )
        ((null i) (values))
...
;;; Vendor independent, PCL/ccl independent ccl functions.
  ...
  (cond ((compiled-function-p object)(get-dump-form object))
        ...
        (T (warn "FROM MAKE LET FORM: object was of bogus type: ~A!!!"
           ...
           (if other-code `(progn ,other-code)
             (progn (warn "there was no code to enccle!") nil)))))
...
(defun MAP-OBJECT (function object)
  ...
  (cond ((circular-list-p object)
        ...
        ((structurep object)(mapstruct function object))
           ...
           (let ((class (find-class class-object nil)))
             (cond
              ((and class (eq (type-of class) 'structure-class))
               (allocate-struct class-object))
              (class (ccl::allocate-instance class))))
           #-(or allegro-v4.1 allegro-v4.2)
           (let ((class-object (find-class class-object nil)))
             (when class-object
               #-(or pcl aclpc)
               (ccl::allocate-instance class-object)
               #+pcl (pcl::allocate-instance class-object)
               #+aclpc(allocate-instance class-object)
               )))
          ((%structurep class-object)
           #-(or aclpc pcl excl)(ccl::allocate-instance class-object)
...
(eval-when (load eval compile)
  ...
  (eval-when (load eval compile)
    ...
    (defmethod ccl::ALLOCATE-INSTANCE ((self ccl:structure-class) 
                                        &rest initargs)
      (declare (ignore initargs))
      (allocate-struct (instance-name self)))

    (defmethod %%ALLOCATE-INSTANCE ((self ccl:structure-class))
      (allocate-struct (instance-name self)))
    ...
    (defmethod ccl::ALLOCATE-INSTANCE ((self symbol) &rest init-plist)
      (allocate-struct self))
  ...
  (eval-when (load eval compile)
    ...
    (defun GET-CLASS-DEFAULT-INITARGS (class)
      ...
      (mapcan #'(lambda (l)
              ...
              #-aclpc(ccl::class-direct-default-initargs class)
   ...
   is available."
      ...
      (mapcar #'ccl::slot-definition-name 
              ;; KOT patch
              ;;(ccl::class-DIRECT-slots (ccl::class-of instance))
              (ccl::class-slots (ccl::class-of instance))
              )
      #+aclpc
  ...
  (eval-when (load eval compile)
    ...
    (defun GET-SLOT-READERS (slot-object)
      (ccl::slotd-readers slot-object))

    (defun GET-SLOT-WRITERS (slot-object)
      (ccl::slotd-writers slot-object))

    (defun GET-SLOT-ALLOCATION (S)
      "Method to get the type of allocation from a standard slot: 
       one of :CLASS or :INSTANCE."
      (let ((alloc (ccl::slotd-allocation s)))
        (cond ((%classp alloc) :CLASS)
              ((member alloc '(:INSTANCE :CLASS)) alloc) 
              (T :INSTANCE))))
    ...
    (defun %CLASSP (X)
      "Predicate, determines whether the object x is a class object."
      (or (typep x 'ccl::standard-class)(typep x 'ccl::built-in-class)))

    (defmethod GET-SLOT-INITFORM (s)
      ""
      (when (slot-boundp s 'ccl::initform)
        (ccl::slotd-initform s)))

    (defun SLOT-HAS-AN-INITFORM-P (s)
      (ccl::slotd-initform s))

    (defun GET-SLOT-INITARGS (s)
      (ccl::slotd-initargs s))

    (defun GET-SLOT-INITARG (s)
      (first (ccl::slotd-initargs s)))

    (defun BUILTIN-CLASS-P (X)
      "Predicate to determine whether class object is a builtin class. returns
       T if it is."
      (and (%classp x)(typep x 'ccl::built-in-class)))

    #+(or allegro-v4.1 allegro-v4.2)
    (defun INSTANCE-P (X)
      "With the 4.0 series, structures are instances as well: exclude these."
      (and (not (typep x 'ccl::structure-class))
           (not (%classp x))(excl::standard-instance-p x)))

    #+allegro-v4.0
    (defun INSTANCE-P (X)
      "With V4.0, structures are instances as well: exclude these:
       NOTE, removed the clause testing for ccl:structure-object."
      (not (%classp x))(excl::standard-instance-p x))
    ...
    ) ;; *** end of non-MCL Allegro (like Sun Allegro) ccl eval-when. ***

  ;; *** lisp Machine Genera 8.x ccl eval-when. ***
  ...
  (eval-when (load eval compile)
    ...
    (defun %GENERIC-FUNCTION-P (x)
      ...
      (ccl-internals::generic-function-p x))
    ...
    (defun BUILTIN-CLASS-P (class-object)
      ...
      (typep class-object 'ccl:built-in-class))
    ...
    (defun INSTANCE-P (x)
      "This will work in Genera 8x ccles: filters out entities that are 
    ...
    (defun GET-SLOT-TYPE (S)
      "This will work for Genera 8x cclses."
      (ccl:slot-definition-type s))

    (defun GET-DIRECT-SLOTS (class-object)
      ""
      (ccl:class-direct-slots class-object))
    ...
    (defun SLOT-HAS-AN-INITFORM-P (slot-object)
      (ccl::slot-definition-initform slot-object))

    (defun GET-SLOT-READERS (slot-object)
      (ccl::slot-definition-readers slot-object))

    (defun GET-SLOT-WRITERS (slot-object)
      (ccl::slot-definition-writers slot-object))
    ...
    (defun GET-SLOT-ALLOCATION (S)
      ...
      or :INSTANCE."
      (let ((alloc (ccl::slot-definition-allocation s)))
        (cond ((%classp alloc) :CLASS)
              ((member alloc '(:INSTANCE :CLASS)) alloc) 
              (T :INSTANCE))))
    ...
    (defmethod GET-SLOT-INITFORM (s)
      ""
      (when (slot-has-an-initform-p s)(ccl::slot-definition-initform s)))

    (defun GET-SLOT-INITARGS (s)
      (ccl::slot-definition-initargs s))

    (defun GET-SLOT-INITARG (s)
      (first (ccl::slot-definition-initargs s)))

    ) ;; end of Genera 8x ccl eval-when.
  ...
  (eval-when (load eval compile)
    ...
    (defun BUILTIN-CLASS-P (X)
      ...
      (and (%classp x)(member (%class-name x)
                              (mapcar #'first ccl-system::built-in-classes) 
                              :test #'equal)))

    ) ;; *** end of Lucid ccl eval-when. ***
  )

;;; THE END OF THE ccl EVAL-WHEN....
...
(defun ALPHABETIZE-BY-KEYWORD (lst)
  ""
  (let ((alpha-cells (lisp:sort (lisp:remove-duplicates (pair-up lst) 
                                                        :test #'equal) #'symf<)))
...
;;; ccl/PCL independent class accessor methods.
...
(defmethod COPY-INSTANCE ((instance T))
  ...
  given ccl instance, writ as a method so youse gys can write ur own."
...
(eval-when (load eval compile)
  ...
  ;; *** Dont-care vendor ccl definitions. ***
 ...
 CLASS-DUMP-FORM:  (NEW)."
    (when (%classp class-object)
      (let ((meta (%class-name (class-of (class-of class-object)))))
        (if (not (equal meta #-aclpc 'ccl::standard-class
  ...
  (defun INSTANCE-NAME (instance)
   ...
   and the type returned is a Common lisp entity, NOT a FLAVOR!"
    (cond ((hash-table-p instance) 'hash-table)
          ...
          (T #-aclpc (ccl::class-name
                      (ccl::class-of instance)
                      #+aclpc(class-name (class-of instance))             
                      ))))

  ) ;; end of ccl eval-when ....

;;; *** Non-lisp machine ccl eval-when. ***
...
(eval-when (load eval compile)
 ...
 this works for most things EXCEPT Genera 8x ccl."
    #-aclpc (ccl::slotd-type s)
    #+aclpc(slot-definition-type s)
    )

;;;(defun GET-DIRECT-SLOTS (class-object)
;;;""
;;;(ccl::class-class-direct-slots class-object))

  (defun %GENERIC-FUNCTION-P (x)
    "Predicate, returns t for generic functions. causes symbol conflict problem
 in genera 8.0."
    #-aclpc(ccl::generic-function-p x)
...
(defun HAS-DUMP-FORM-P (class-name)
  ...
  (lisp:get class-name #+mcl 'cl-user::%%DUMP-FORM-METHOD%%
...
(defmacro DEFINE-DUMP-FORM (class-name arglist &body body)
  ...
  `(progn (setf (lisp:get ',class-name #+mcl 'cl-user::%%dump-form-method%%
                     #-mcl 'user::%%dump-form-method%%
                     ) T)
...
(defun GET-SLOT-VALUES (ccl-instance)
  "given a pcl/ccl instance,constructs a plist of all the saveable
   slot/value pairs."
  (incf *global-instance-count*)
  (let ((unsaveable (get-unsaveable-slotnames ccl-instance)))
    (%flatten (lisp:remove-duplicates (pair-up
...
""
  ...
  (let* ((*print-level*  max-print-level)
    ...
    (setf (lisp::get '.%%SL%%. 'namecounter) 0)
...
(defun %EVERY (predicate sequence)
  "New definition of the every predicate. In MCL, for example, EVERY infinite loops
   when it encounters a circular list. This one doesnt."
...
(setf *global-unsaveable-slotnames* '(CCL::FRED-HISTORY))
...
(defun FUNCTION-P (x)
"Predicate: returns t if sometghing is a compiled function form, or a LAMBDA."
  (and (consp x)
       (eq (car x) 'function)
       (consp (cadr x))
...
""
  (let ((table (lisp::get fn-name 'memo)))
...
(progn
  ...
  (cond 
   ((ccl::handlep object)(write-tag +handle-tag+ stream)
...
(defun %SAVE-ACTUAL-OBJECT-1 (object table)
 ...
 (if (member object  *global-unsaveable-slotnames* :test #'equal)
   ...
   (progn 
  (when *wotf-debug* (format t "SAVING: ~A~%" object))
  (cond ((ccl::handlep object)(push-tag +handle-tag+)
